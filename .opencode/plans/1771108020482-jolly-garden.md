...
- If blocked, document blocker + fallback in `next-steps.md` and continue all non-blocked work.
...
- Commit 2: add `DuckGame.Net8.csproj` + `Steam.Net8.csproj` + initial restore/buildable structure.
- Commit 3: compile-compatibility cleanup (namespace/import fixes, proxy shims, net8 startup stub).
- Commit 4: runtime boot fixes (Steam-off guards, crash path guards, font + texture net8-safe paths).
- Commit 5: publish/packaging implementation (`osx-arm64` native staging + publish script).
- Commit 6: stabilization/docs/final tracking sync after full verification.
