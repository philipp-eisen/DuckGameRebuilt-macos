        ...
        public override void Activate()
        {
            ...
            for (int y = 0; y < yM; y++)
            {
                for (int x = 0; x < xM; x++)
                {
                    using (MemoryStream ms = new MemoryStream())
                    {
                        using Bitmap botmop = bm.Clone(new System.Drawing.Rectangle(8 * x, 8 * y, 8, 8), PixelFormat.DontCare);
                        botmop.Save(ms, ImageFormat.Png);
                        ms.Position = 0;
                        Sprite s = new Sprite(new Tex2D(TextureConverter.LoadPNGWithPinkAwesomeness(Graphics.device, ms, false), "shatter"));
