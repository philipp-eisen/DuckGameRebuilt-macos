...
cleanup_dotnet_artifacts() {
  rm -f "$ROOT_DIR/DuckGame/obj/project.assets.json"
  rm -f "$ROOT_DIR/DuckGame/obj/project.nuget.cache"
  rm -f "$ROOT_DIR/DuckGame/obj"/DuckGame.Net*.csproj.nuget.*

  rm -f "$ROOT_DIR/Steam/obj/project.assets.json"
  rm -f "$ROOT_DIR/Steam/obj/project.nuget.cache"
  rm -f "$ROOT_DIR/Steam/obj"/Steam.Net*.csproj.nuget.*
}
...
echo "[2/5] Cleaning conflicting dotnet restore artifacts"
cleanup_dotnet_artifacts

echo "[3/5] Building native macOS libraries"
cmake -S "$ROOT_DIR/FNA/lib/FAudio" -B "$ROOT_DIR/FNA/lib/FAudio/build" -DCMAKE_BUILD_TYPE=Release
cmake --build "$ROOT_DIR/FNA/lib/FAudio/build" --config Release --parallel "$CPU_COUNT"

cmake -S "$ROOT_DIR/FNA/lib/FNA3D" -B "$ROOT_DIR/FNA/lib/FNA3D/build" -DCMAKE_BUILD_TYPE=Release
cmake --build "$ROOT_DIR/FNA/lib/FNA3D/build" --config Release --parallel "$CPU_COUNT"

make -C "$ROOT_DIR/FNA/lib/Theorafile"

echo "[4/5] Building DuckGame ($CONFIGURATION) with Mono"
xbuild \
  /p:Configuration="$CONFIGURATION" \
  /p:SolutionDir="$ROOT_DIR/" \
  "$ROOT_DIR/DuckGame/DuckGame.csproj"

echo "[5/5] Copying runtime macOS libraries"
...
echo "Build complete."
echo "Run with: cd \"$ROOT_DIR/bin\" && mono ./DuckGame.exe -nosteam"
